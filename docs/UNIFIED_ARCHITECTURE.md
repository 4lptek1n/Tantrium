...
**Sıra kuralı:** alttan üste (L0→L6). **Tamamlanan (2026-06):** F0 (NC Möbius κ) + F0b (bounded_kappa_distance) + F1/F5 encoder collision KÖK çözüm (imza-encoding + manifold migrasyonu) + F2 (flywheel) + F2b (ask tek grounding) + F3-çekirdek (admit() + parity) + F4 (Producer çatısı + #8 konveks-çekirdek + engine.grow→deduce + Wonder loop) + F6-çekirdek (serve smoke) + 4 gerçek dedup (#7/#9/#10 tam, #8 kısmî). Test 443→509+.
