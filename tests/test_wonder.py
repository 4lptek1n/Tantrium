...
from tantrium.reasoning.wonder import _SYNTHETIC_SOURCES, WonderScore, WonderScorer
...
def test_score_components_present():
    ...
    m = SemanticManifold()
    m.add_unchecked(
        Concept(name="theorem:x", moments=_moments(0.3), domain="math", source="theorem_graph")
    )
    eng = _FakeEngine(m)
    gap = Gap(
        signal="grid",
        name="g",
        description="d",
        location=[float(x) for x in _moments(0.3)],
        priority=1.0,
    )
...
def test_all_synthetic_sources_penalized():
    ...
    for src in _SYNTHETIC_SOURCES:
        ...
        for i in range(5):
            m.add_unchecked(
                Concept(
                    name=f"{src}_{i}", moments=_moments(0.4 + i * 0.001), domain="x", source=src
                )
            )
...
def test_rank_sorts_descending():
    ...
    m = SemanticManifold()
    m.add_unchecked(
        Concept(name="theorem:a", moments=_moments(0.3), domain="math", source="theorem_graph")
    )
    m.add_unchecked(
        Concept(name="⟨bridge:x⟩", moments=_moments(0.7), domain="genesis", source="bridge")
    )
    eng = _FakeEngine(m)
    gaps = [
        Gap(
