...
def test_loop_cycle_exported_from_tantrium():
    ...
    cycle = LoopCycle(
        ...
        concepts_before=100,
        concepts_after=100,
        tau_edges_before=500,
        tau_edges_after=500,
        necessity_edges_before=10,
        necessity_edges_after=10,
        duration_s=0.1,
    )
...
def test_derive_handles_min_moment_length(ai):  # type: ignore[misc]
    ...
    c_long = Concept(
        name="_test_long_", moments=[Fraction(1, 2) ** k for k in range(8)], domain="test"
    )
    c_short = Concept(
        name="_test_short_", moments=[Fraction(1, 2) ** k for k in range(4)], domain="test"
    )
...
def test_bridge_distances_reflect_bridge_concept(ai):  # type: ignore[misc]
    ...
    result = ai.bridge("theorem", "proof")
    min(len(result.bridge_moments), len(result.source_distance.__class__.__mro__))
