...
class TestCombination:
    ...
    def test_combination_off_flag(self, pe):
        ...
        cert = pe.produce("egfr", max_steps=4, beam_width=2,
                          combination=False, inject=False, top_k=3)
...
class TestCertificate:
    ...
    def test_certificate_fields(self, pe):
        ...
        assert cert.verdict in ("İŞE YARAYABİLİR", "İŞE YARAMAZ",
                                "KISMÎ", "ÜRETİLEMEDİ", "GEÇERSİZ")
    ...
    def test_to_cure_dict(self, pe):
        cert = pe.produce("alzheimer", max_steps=4, beam_width=2,
                          inject=False, top_k=3)
        d = cert.to_cure_dict()
        assert "disease" in d
...
class TestDeterminism:
    ...
    def test_same_input_same_output(self, pe):
        ...
        def run():
            return pe.produce("egfr", max_steps=4, beam_width=2,
                              refine_rounds=0, inject=False, top_k=3)
        c1 = run()
        c2 = run()
    ...
    def test_smiles_deterministic(self, pe):
        def run():
            return pe.produce(ERLOTINIB, max_steps=4, beam_width=2,
                              refine_rounds=0, inject=False, top_k=3)
        c1 = run()
        c2 = run()
        assert c1.designed_smiles == c2.designed_smiles
...
class TestAIProduce:
    ...
    def test_ai_produce_passthrough(self, ai):
        ...
        cert = ai.produce("egfr", max_steps=4, beam_width=2,
                          refine_rounds=0, inject=False, top_k=3)
        assert isinstance(cert, ProductionCertificate)
        assert cert.target == "egfr"

    def test_ai_produce_smiles(self, ai):
        cert = ai.produce(ERLOTINIB, max_steps=4, beam_width=2,
                          refine_rounds=0, inject=False, top_k=3)
