...
def test_recovers_hidden_structure_exactly():
    ...
    obs = [
        sum(w * (x**k) for x, w in zip(hidden_support, hidden_weights, strict=False))
        for k in range(10)
    ]
    rec = reconstruct_measure(obs, max_atoms=4)
    assert rec.rank == 3, "3 mod üretiyor"
    assert rec.reconstruction_error < 1e-6
    for got, want in zip(sorted(rec.support), hidden_support, strict=False):
...
def test_raw_hankel_reads_structure():
    ...
    random.seed(1)
    structured = [
        math.sin(0.3 * t) + 0.5 * math.sin(0.7 * t) + 0.3 * math.sin(1.3 * t) for t in range(64)
    ]
