...
from tantrium.core.enrichment import (
    _DIMENSIONS,
    enrich_concept,
    fetch_molecular_smiles,
    fetch_physical_properties,
)
...
class _AI:
    ...
    def ground_full(self, name, *, molecule=None, dna=None, **kw):
        b = {}
        if molecule:
            self.bindings.append((name, "HAS_COMPOUND", None))
            b["HAS_COMPOUND"] = "p"
        if dna:
            self.bindings.append((name, "HAS_DNA", None))
            b["HAS_DNA"] = "p"
...
def test_multiple_complementary_dimensions():
    ...
    ai = _AI()
    r = enrich_concept(
        ai, "egfr", protein="MEEPQSDP", dna="ATCGATCG", properties=[1.0, 2.0, 3.0], network=False
    )
...
def test_dims_filter_restricts():
    ...
    ai = _AI()
    r = enrich_concept(
        ai, "egfr", protein="MEEPQSDP", smiles="CC(=O)O", network=False, dims=["protein"]
    )
...
def test_no_molecule_3d_dimension():
    ...
    assert (
        "smiles" in by_key["molecule"].fetch.__doc__ if by_key["molecule"].fetch.__doc__ else True
    )
    ...
    class _LawAI:
        def discover_law(self, seq):
            return type(
                "LD",
                (),
                {"law_holds": True, "order": 2, "recurrence": [1.0, 1.0], "modes": [-0.618, 1.618]},
            )()
