...
def _fit_modular(train):
    ...
    for m in range(2, 13):
        ...
        if len(offs) == 1:
            ...
            return InventedPrimitive(f"mod{m}+{c}", ps,
                                     (lambda m=m, c=c: (lambda x: x % m + c))(), "modular")
    return None
...
def _fit_power(train):
    ...
    for k in range(3, 7):                          # **2 zaten tabanda; 3+ yeni
        if all(y == x ** k for x, y in train):
            return InventedPrimitive(f"pow{k}", f"({{c}}) ** {k}",
                                     (lambda k=k: (lambda x: x ** k))(), "power")
        if all(y == x ** k - x for x, y in train):
            return InventedPrimitive(f"pow{k}_id", f"(({{c}}) ** {k}) - ({{c}})",
                                     (lambda k=k: (lambda x: x ** k - x))(), "power")
    return None
...
def invent_primitive(examples, *, register: bool = True) -> "InventedPrimitive | None":
    ...
    try:
        if not all(isinstance(e[0], int) and isinstance(e[1], int)
                   and not isinstance(e[0], bool) for e in examples):
            return None
    except Exception:
        return None
    best = None
