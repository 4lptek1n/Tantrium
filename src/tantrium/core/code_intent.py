...
def _run_chain(program: str, inputs: list) -> list:
    ...
    from tantrium.core.code_synthesis import _SENTINEL, _run
...
def derive_spec(intent: str, *, research: bool = True) -> DerivedSpec:
    ...
    """
    from tantrium.core.nl_code import nl_to_program, parse_binary, parse_operations
    ...
    if not ops:
        ...
        if match is not None:
            ...
            if ds.grounded:
                return ds
        ds.clarify = (
            "Bu isteği bir operasyona bağlayamadım. Bir örnek ver (girdi → çıktı) ki "
    ...
    if not ds.grounded:
        ds.clarify = (
            "İsteği anladım ama uygun girdi tipini çıkaramadım. Bir örnek ver "
    ...
    if _re_mod.search(pattern, g, _re_mod.IGNORECASE):  # (1) açık bağlaç
        ...
        for idx, part in enumerate(parts):
            ...
            if ds.grounded and ds.examples:
                ...
                specs.append(
                    {
                        "name": _safe_name(base, idx, used),
                        "examples": ds.examples,
                        "understood": ds.understood,
                        "part": part,
                    }
                )
        return specs
