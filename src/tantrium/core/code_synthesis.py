...
def synthesize(examples, *, max_depth: int = 5, beam_width: int = 10,
    ...
    for depth in range(1, max_depth + 1):
        ...
        for c in beam:
            ...
            # adayı argümanlarla birleştir (a=c, b=arg) — tek-argümanda da (polinom: x²+x→(x+1)*x),
            # çok-argümanda da (x op y). Sayısal girdide ikili op anlamlı.
            sample0 = examples[0][0]
            v0 = list(sample0) if isinstance(sample0, tuple) else [sample0]
            if any(isinstance(v, (int, float)) and not isinstance(v, bool) for v in v0):
                for other in argnames:
                    for p in _BINARY:
                        exprs.append(p.format(a=c, b=other))
            for expr in exprs:
                if expr in seen:
                    continue
                seen.add(expr)
