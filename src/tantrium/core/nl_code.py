...
_OP_VOCAB: list[tuple[tuple, str, str]] = [
    ...
    (
        "map_double",
        ("her birini iki kat", "her elemanı iki kat", "double each", "map double"),
        "[i * 2 for i in ({c})]",
    ),
    (
        "map_square",
        ("her birinin karesi", "her elemanın karesi", "square each"),
        "[i * i for i in ({c})]",
    ),
    (
        "filter_pos",
        ("pozitifleri", "sadece pozitif", "filter positive", "positive only"),
        "[i for i in ({c}) if i > 0]",
    ),
    (
        "filter_even",
        ("çiftleri", "sadece çift", "even only", "filter even"),
        "[i for i in ({c}) if i % 2 == 0]",
    ),
    (
        "sum_squares",
        ("karelerin toplamı", "kareleri topla", "sum of squares"),
        "sum([i * i for i in ({c})])",
    ),
    (
        "average",
        ("ortalaması", "ortalama", "average", "mean", "ortalamasını"),
        "sum({c}) / len({c})",
    ),
    ...
    (
        "increment",
        ("bir ekle", "bir artır", "artır", "increment", "add one", "plus one"),
        "({c}) + 1",
    ),
    ...
    (
        "reverse",
        ("tersine çevir", "ters çevir", "tersine", "ters", "reverse", "reversed"),
        "({c})[::-1]",
    ),
    ...
    (
        "length",
        ("uzunluğu", "uzunluk", "kaç eleman", "eleman sayısı", "length", "count", "size"),
        "len({c})",
    ),
...
_BINARY_VOCAB: list[tuple[str, tuple, str]] = [
    (
        "add",
        ("topla", "toplama", "toplamı", "ekle", "add", "addition", "plus", "sum"),
        "({a}) + ({b})",
    ),
    (
        "subtract",
        ("çıkar", "çıkarma", "çıkart", "fark", "subtract", "subtraction", "minus"),
        "({a}) - ({b})",
    ),
    (
        "multiply",
        ("çarp", "çarpma", "çarpım", "multiply", "multiplication", "times", "product"),
        "({a}) * ({b})",
    ),
