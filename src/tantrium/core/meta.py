...
class GraphAdapter:
    ...
    def candidates(self, engine, *, priority=None, **kw) -> list[MetaCandidate]:
        ...
        from tantrium.reasoning.causal_rules import (
            ...
            register_implication_rule,
            register_transitive_rule,
        )
        ...
        for a in order:
            ...
            for e1 in el:
                ...
                for eb in tau.edges.get(b, []):
                    if str(getattr(eb, "target", "")) == a and is_semantic(
                        getattr(eb, "paradigm", "")
                    ):
        ...
        for relX, prs in relX_pairs.items():
            ...
            for relY in sorted(common):
                cands.append(
                    self._make_implication_candidate(
    ...
    def _make_implication_candidate(
        self, relX, relY, pairs, pair_rels, register_fn
    ) -> MetaCandidate:
        def build(train):
        ...
        return MetaCandidate(
            name=f"{relX}⊑{relY}", build=build, instances=list(pairs), verify=verify, commit=commit
        )
...
def apply_converse_rules(engine, *, max_apply: int = 100) -> int:
    ...
    for a in list(tau.edges):
        ...
        for e in list(tau.edges.get(a, [])):
            ...
            if any(
                str(getattr(x, "target", "")) == a and getattr(x, "paradigm", "") == relY
                for x in back
            ):
                continue
            ...
            if manifold is not None:
                ...
                if cb is not None and ca is not None:
                    r = certify_transition(
                        [float(m) for m in cb.moments], [float(m) for m in ca.moments], min_depth=2
                    )
                    if not r.on_path:
                        continue
...
def apply_implication_rules(engine, *, max_apply: int = 100) -> int:
    ...
    for a in list(tau.edges):
        ...
        for e in list(el):
            ...
            b = str(getattr(e, "target", ""))
            if any(
                str(getattr(x, "target", "")) == b and getattr(x, "paradigm", "") == relY
                for x in el
            ):
                continue
            if manifold is not None:
                ...
                if ca is not None and cb is not None:
                    r = certify_transition(
                        [float(m) for m in ca.moments], [float(m) for m in cb.moments], min_depth=2
                    )
                    if not r.on_path:
                        continue
...
def derive_analogy_edges(engine, *, min_shared: int = 3, max_apply: int = 30) -> int:
    ...
    def typed_neighbors(name):
        return {
            (getattr(e, "paradigm", ""), str(getattr(e, "target", "")))
            for e in tau.edges.get(name, [])
            if is_semantic(getattr(e, "paradigm", ""))
        }
        ...
        for Y in nodes[i + 1 :]:
            ...
            for relR, Z in nx:  # X'in ilişkisini analog Y'ye transfer et (eksikse)
                ...
                if cz is None:
                    continue
                r = certify_transition(
                    [float(m) for m in cy.moments], [float(m) for m in cz.moments], min_depth=2
                )
    ...
    def candidates(self, engine, *, examples=None, **kw) -> list[MetaCandidate]:
        ...
        for name, builder in _CANDIDATE_SCHEMAS:
            ...
            def _mk(name=name, builder=builder):
                ...
                return MetaCandidate(
                    name=name, build=build, instances=ex, verify=verify, commit=commit
                )
