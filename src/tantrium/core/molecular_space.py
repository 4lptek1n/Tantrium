...
DRUG_LIBRARY: list[tuple[str, str, str]] = [
    ...
    (
        "temsirolimus",
        "COC1CC(=O)CC(CC(CC(=O)C(CC(CC(OC(=O)C(CC(C1OC)OC)C)C)O)C)C(=O)OCC(O)CO)OC",
        "kinase",
    ),
    ...
    (
        "azithromycin",
        "CC[C@@H]1OC(=O)[C@H](C)[C@@H](O[C@@H]2C[C@@](C)(OC)[C@@H](O)[C@H](C)O2)[C@H](C)[C@@H](O)[C@](C)(OC(=O)CC(C)C)C[C@@H](C)[C@@H]1O",
        "antibiotic",
    ),
    ("metronidazole", "Cc1ncc([N+](=O)[O-])n1CCO", "antibiotic"),
    (
        "doxycycline",
        "CN(C)[C@@H]1C(=O)C(C(N)=O)=C(O)[C@@]2(O)C(=O)C3=C(O)c4c(O)cccc4[C@](C)(O)[C@H]3[C@@H]12",
        "antibiotic",
    ),
    ...
    (
        "digoxin",
        "CC1OC(OC2CC(OC3CC(O)CC(O3)C3CCC4(CC3)C3CCC5(O)CC(O)CC5C3CC4=O)CC2O)CC1O",
        "cardio",
    ),
    ...
    (
        "paclitaxel",
        "CC1=C2[C@@]([C@H](C(=O)[C@@H]3[C@@H]([C@]2(OC(=O)c2ccccc2)[C@@H](C([C@H]3OC(C)=O)(C)C)O)OC(=O)[C@@H](NC(=O)c2ccccc2)c2ccccc2)O)(C[C@@H]1OC(=O)[C@H](O)[C@@H](NC(=O)c1ccccc1)c1ccccc1)O",
        "oncology",
    ),
    ("methotrexate", "CN(Cc1cnc2nc(N)nc(N)c2n1)c1ccc(NC(=O)C(CCC(=O)O)C(=O)O)cc1", "oncology"),
    (
        "doxorubicin",
        "COc1cccc2C(=O)c3c(O)c4C[C@](O)(CC(=O)CO)[C@@H](O[C@H]5C[C@@H](N)[C@@H](O)[C@@H](C)O5)Cc4c(O)c3C(=O)c12",
        "oncology",
    ),
    ...
    (
        "lopinavir",
        "Cc1ccc2cc(CC(=O)N[C@@H](Cc3ccccc3)[C@@H](O)CN3C[C@H]4CCCC[C@@H]4C3=O)ccc2n1",
        "antiviral",
    ),
    (
        "remdesivir",
        "CCC(CC)COC(=O)[C@@H](N)P(=O)(OC[C@H]1O[C@@](C#N)([C@H](O)[C@@H]1O)c1ncnc2N[C@@H](C)c12)Oc1ccccc1",
        "antiviral",
    ),
    # --- Nöroloji ---
...
class MolecularSpace:
    ...
    def morph(
        ...
        for i in range(steps):
            ...
            interp = [(1 - t) * a + t * b for a, b in zip(src_m, tgt_m, strict=False)]
    ...
    def lineage(self, smiles: str, depth: int = 3) -> list[list[MolPoint]]:
        ...
        def _nearest(moments: list[float], exclude: set[str], k: int) -> list[MolPoint]:
            ...
            for name, (smi, cls, mom) in lib.items():
                ...
                scored.append(
                    MolPoint(name=name, smiles=smi, cls=cls, moments=mom, w2_to_target=w2)
                )
        ...
        for _ in range(depth):
            ...
            if layer:
                avg = [
                    sum(layer[j].moments[i] for j in range(len(layer))) / len(layer)
