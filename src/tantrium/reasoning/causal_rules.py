...
def derive_transitive_hypotheses(
    engine, *, max_seeds: int = 12, max_hyps: int = 10, sturm_check: int = 6
) -> list[dict]:
            ...
            for e1 in cz:
                ...
                for e2 in tau.edges.get(b, []):
                    ...
                    if (
                        ...
                        or c == s
                        or c == b
                        or c.lower() in GENERIC_TERMS
                        or b.lower() in GENERIC_TERMS
                    ):
                        continue
                    key = (s, derived, c)
                    if key in seen:
                        continue
                    if any(
                        str(getattr(e, "target", "")) == c and getattr(e, "paradigm", "") == derived
                ...
                if ca is not None and cc is not None:
                    try:
                        _ok, pmin = pe._sturm_path_pivot_min(
                            [float(m) for m in ca.moments], [float(m) for m in cc.moments]
                        )
