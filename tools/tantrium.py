#!/usr/bin/env python3
"""Tantrium Proof Foundry v1.1 command line entrypoint."""
...
def register_kernel_artifacts(ell: int, atlas: AtlasDB) -> None:
...
def run_structure_miner(ell: int, mixed: Path, atlas: AtlasDB, q_mode: str) -> None:
    if not mixed.exists(): return
    summary=mine(mixed,q_mode); report=Path(f'results/engine/ell{ell}_structure_miner_report.md')
    write_report(report,summary)
    atlas.register_structure_report(f'ell{ell}_structure',f'ell{ell}_mixed_depth',str(report),summary)
    print(f'wrote {report}')

def cmd_graph(args):
    store=GraphStore(args.graph_store)
    graph=store.load()
...
def cmd_status(args):
    ...
    if args.update: graph=store.update_from_atlas(atlas.manifest())
    else: graph=store.load()
    out=Path(args.output); out.parent.mkdir(parents=True,exist_ok=True)
    out.write_text(graph.markdown()+"\n\n"+atlas.status_table())
...
def cmd_build_kernel(args):
    ...
    run_cmd(cmd)
    register_kernel_artifacts(ell,atlas)
    run_structure_miner(ell,Path(f'results/engine/ell{ell}_mixed_depth_kernel.csv'),atlas,args.q_mode)
...
def certify_one(input_path: Path, q_target: int, q_mode: str, source_policy: str, model: str, theorem_id: str, kernel_id: str, output: Path, atlas_root: str, graph_store: str, allow_q_down=False, allow_diff_down=False):
    ...
    if not ok:
        ...
        obs_id=f'{theorem_id}_obstruction'
        atlas.register_obstruction(obs_id,theorem_id,kernel_id,str(mass),coords)
        GraphStore(graph_store).add_obstruction(obs_id,f'Obstruction for {theorem_id}',artifacts=[str(output)],note=str(coords))
    else:
...
def ensure_kernel(ell: int, args) -> Path:
    ...
    try:
        ...
        print(f'build failed for ell={ell}: {exc}')
    return mixed
...
def cmd_scan(args):
    ...
    for ell in range(1,args.max_ell+1):
        ...
        run_structure_miner(ell,mixed,atlas,args.q_mode)
...
def main():
    parser=argparse.ArgumentParser(description='Tantrium Proof Foundry v1.1'); sub=parser.add_subparsers(dest='cmd',required=True)
